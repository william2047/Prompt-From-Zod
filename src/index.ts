...
type CompatibleZodObject<D extends N = 5> = ZodObject<
  Record<string, CompatibleZodTypes<Dec<D>>>,
  core.$strip
>;
...
type InputPimaryPrompt<S extends CompatibleZodTypes> = (
  message: string,
  schema: S,
 ...
 */
function zodParseToValidate<S extends CompatibleZodTypes>(
  schema: CompatibleZodTypes,
): (value: z.infer<S>) => Promise<boolean | string> {
...
const booleanPrompt: InputPimaryPrompt<ZodBoolean> = async (
  message,
  schema,
  abortController?,
): Promise<boolean> => {
...
const stringPrompt: InputPimaryPrompt<ZodString> = async (
  message,
  schema,
  abortController?,
): Promise<string> => {
  ...
  const metaType =
    typeof schema.meta()?.type === "string"
      ? ((schema.meta() as { type: string }).type as string).toLowerCase()
      : undefined;
  if (
    metaType &&
    (metaType === "longtext" ||
...
const numberPrompt: InputPimaryPrompt<ZodNumber> = async (
  message,
  schema,
  abortController?,
): Promise<number> => {
...
const enumPrompt: InputPimaryPrompt<ZodEnum> = async (
  message,
  schema,
  abortController?,
...
async function arrayPrompt<S extends CompatibleZodArray>(
  message: string,
  schema: S,
  indentCount: number,
): Promise<z.infer<S["element"][]>> {
...
async function schemaWalker<S extends CompatibleZodTypes, T = z.infer<S>>(
  schema: S,
  propertyLabel?: InputLabelsForSchema<S>,
  indentCount: number = 0,
): Promise<T> {
  // Case by case, handle the schema type
  switch (true) {
    case schema instanceof ZodBoolean:
      return (await booleanPrompt(
        getInputMessage(indentCount, "Boolean", propertyLabel as InputLabelObject | string),
        schema,
      )) as T;
    case schema instanceof ZodString:
      return (await stringPrompt(
        getInputMessage(indentCount, "String", propertyLabel as InputLabelObject | string),
        schema,
      )) as T;
    case schema instanceof ZodNumber:
      return (await numberPrompt(
        getInputMessage(indentCount, "Number", propertyLabel as InputLabelObject | string),
        schema,
      )) as T;
    case schema instanceof ZodEnum:
      return (await enumPrompt(
        getInputMessage(indentCount, "Enum", propertyLabel as InputLabelObject | string),
        schema,
      )) as T;
    ...
    case schema instanceof ZodObject:
      ...
      for (const key in schema.shape) {
        object[key as keyof T] = await schemaWalker(
          schema.shape[key],
          propertyLabel
            ? (propertyLabel as InputLabelsForSchema<CompatibleZodObject>)[key]
            : undefined,
          indentCount + 1,
        );
      }
    ...
    case schema instanceof ZodArray:
      ...
      const arr = await arrayPrompt(
        getInputMessage(
          indentCount + 1,
          "Array of values",
          propertyLabel as InputLabelObject | string,
 ...
 * });
 */
async function promptsFromZod<S extends CompatibleZodTypes>(
  schema: S,
  propertyLabel?: InputLabelsForSchema<S>,
  doConfirm: boolean = true,
): Promise<z.infer<S>> {
